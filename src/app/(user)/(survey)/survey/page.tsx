        ...
        <Bordered className="mb-10 w-4/5 grow">
          <div className="m-5 flex space-x-6">
            <div className="flex flex-1 flex-col items-center justify-center">
              <p>
                An welchem Wochentag gehen Sie für gewöhnlich einkaufen?
              ...
              <div className="hint flex p-1 pl-5 text-left opacity-30">
                ...
                <p>
                  Sie können mehrere Tage auswählen, jedoch wird anschließend nur eine Einkaufsstrategie
                  für alle ausgewählten Tage angewendet. Sie können die Umfrage für verschiedene Tage
                  wiederholen.
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-1 flex-col items-center">
              <p>
                Zu welcher Tageszeit gehen Sie für gewöhnlich einkaufen?
                <span className="p-2" style={{ color: "red" }}>
                  *
                </span>
              </p>
              ...
              <Select
                ...
                className="mt-5 p-2"
                ref={selectedTimeRef}
                defaultValue={formState.time.length == 0 ? "X" : formState.time}
                onChange={(e) => {
                  timeRequiredErrorRef.current!.style.display = "none";
                  onChange(e);
                }}
              >
            ...
            <button
              ...
              onClick={(_) => {
                ...
                if (style.transform === caretDown.degree) {
                  ...
                  optionalQuestionsRef.current!.style.display = "flex";
                } else {
                  style.transform = caretDown.degree;
                  style.marginTop = caretDown.marginTop;
                  optionalQuestionsRef.current!.style.display = "none";
                }
              }}
            >
              <span
              ...
              <span className=" text-buttonBorderColor">(optional)</span>
            </button>
            <div
              style={{ display: "none" }}
            ...
            >
              ...
              <div className="m-5 flex w-full grow space-x-6">
                ...
                <div className="flex flex-1 flex-col items-center justify-between space-y-3">
                  <p>Für wie viele weitere Personen kaufen sie ein?</p>
                  <small className="flex items-start space-x-8">
                    <div ref={buyingForSomeRef}>
                      {buyingFor.slice(0, -1).map((x) => {
                        return (
                          <Label className="flex justify-between" key={x.value}>
                            {x.label}
                            <Input
                              disabled={formState.nobody}
                              onChange={onChange}
                              name={x.value}
                              value={(formState as any)[x.value]}
                              type="number"
                              min="1"
                              max="99"
                              className="ml-2 w-[fit-content]"
                    ...
                    <div>
                      {buyingFor.slice(-1).map(
                        checkboxMapper(
                          ...
                          (e: ChangeEvent<HTMLInputElement>) => {
                            const self = e.target as HTMLInputElement;
                            const others = buyingForSomeRef.current!.querySelectorAll("input");
                            if (self.checked) {
                              let updateState: any = {
                                ...formState,
                              };
                  ...
                  />
                  <small className="mb-5 mt-3 flex flex-wrap pl-5 pr-8 text-center">
                    <div className="flex" ref={allergiesRef}>
                      {allergies.slice(0, -2).map(
                        checkboxMapper("allergies", onChange, formState.noAllergies)
                      )}
                    </div>
                    <div>
                      {[allergies[allergies.length - 2]].map(
                        checkboxMapper(
                          ...
                          (e: ChangeEvent<HTMLInputElement>) => {
                            const self = e.target;
                            let allergies = allergiesRef.current!.querySelectorAll("input");
                            if (self.checked) {
                              let updateState: any = {
                                ...formState,
                              };
                              allergies.forEach((x) => {
                                updateState[x.value] = false;
                              });
                              updateState["otherAllergies"] = "";
                              onChange(e, updateState);
                            } else {
                              onChange(e);
                            }
                    ...
                    <div ref={typedAllergiesRef}>
                      {(() => {
                        const others = allergies[allergies.length - 1];
                        return (
                          <Label key={others.value}>
                            ...
                            <Input
                              ...
                              disabled={formState.noAllergies}
                              pattern="(\w* ?)*"
                              name="otherAllergies"
                            />
                          </Label>
